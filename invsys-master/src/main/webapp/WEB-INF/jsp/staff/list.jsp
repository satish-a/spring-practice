...
Ext.onReady(function() 
{
	...
	var userStore = Ext.create('Ext.data.Store', 
	{
		...
		{
			type : 'ajax',
			api : { read : '<c:url value="/staff/list"/>' },
	...
	var userList = Ext.create('Ext.grid.Panel',
	{
		...
		[
			{ text : 'Account Name', flex : 1, sortable : true, filterable: true, dataIndex : 'username',
				...
				{
					return '<a href="<c:url value="/staff/view/"/>' + value + '">' + value + '</a>';
				}
			}, 
		...
		[
			{	items : 
				[
					'-',
					{ text : 'Add', itemId : 'add', 
						handler: function() { window.location = "<c:url value="/staff/add"/>" ;}
					}, 
					'-',
				...
				displayMsg : 'Displaying staff members {0} - {1} of {2}', emptyMsg : "No staff members to display"
	...
	function processMarkedDeletions()
	{
		...
		if (sels.length > 0) 
		{
			...
			Ext.Msg.show(
			{
				title: 'Delete Staff Members?', buttons: Ext.MessageBox.YESNO, msg: 'Do you want delete the selected staff members?',
				fn: function(btn)
				{
					if (btn == 'yes')
					{
	...
	function sendMarkedDeletions(users)
	{
		Ext.Ajax.request(
		{ 
			url: '<c:url value="/staff/delete"/>', method:'POST', params: { 'users': Ext.JSON.encode(users) },
	...
	{
		title : 'Staff List', labelWidth : 80, frame : true, collapsible:true, collapsed:true, monitorValid : true,
