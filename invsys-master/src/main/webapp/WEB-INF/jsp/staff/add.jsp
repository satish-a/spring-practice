...
Ext.onReady(function() 
{
	...
	{
		bodyStyle:'padding:5px', labelWidth : 80, monitorValid : true, fieldDefaults: { labelAlign: 'left', msgTarget: 'side' },
		defaults: { anchor: '100%' }, url : '<c:url value="/staff/add" />',
		items : 
		[
			{ 	title: 'Personal Details', layout:'column', bodyStyle:'padding:10px', border:false,
				...
				[ 
					{ 	columnWidth:.5, border:false, layout: 'anchor', defaultType: 'textfield',
						items: 
						[
							{ fieldLabel: 'Account Name', name : 'username', value : '<c:out value="${userBean.username}" />', anchor:'95%', allowBlank : false }, 
							{ fieldLabel: 'Account Type', name : 'accountType', value : 'Staff', anchor:'95%', readOnly: true }
						]
					},
					{ 	columnWidth:.5, border:false, layout: 'anchor', defaultType: 'textfield',
						items: 
	...
	function userDetailsSubmit()
	{
		userDetailsForm.getForm().submit(
		{
			...
			{
				window.location = "<c:url value="/staff"/>" ;
			},
