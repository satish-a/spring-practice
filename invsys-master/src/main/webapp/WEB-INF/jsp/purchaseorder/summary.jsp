...
Ext.onReady(function () {
	...
	var allSummaryStore = Ext.create('Ext.data.Store', 
	{
		...
		proxy : 
		{ 
			type : 'ajax', api : { read : '<c:url value="/purchaseorder/allSummary"/>' },
			reader : { type : 'json', root : 'data', successProperty : 'success' },
			listeners : 
			{
				exception : function(proxy, response, operation) 
	...
	var tabPanel = Ext.create('Ext.tab.Panel', 
	{
		...
		items: 
		[
			{ title:'Monthly Purchase', items: [monthlySummary] },
			{ title:'Yearly Purchase', items: [yearlySummary] },
			{ title:'All Purchase', items: [allSummary] }
		]
	});
