...
Ext.onReady(function() 
{
	...
	{
		model : 'Product', remoteFilter : true, remoteSort : true, autoLoad : true,
		proxy : 
		{
			type : 'ajax', api : { read : '<c:url value="/product/list"/>' },
	...
	{
		model : 'PurchasedItems', autoLoad : true, groupField: 'order',
		proxy : 
		{
			type : 'ajax', api : { read : '<c:url value="/purchaseorder/view/${poBean.id}/itemList"/>' },
			reader : { type : 'json', root : 'data', successProperty : 'success' },
			listeners : 
			{
				exception : function(proxy, response, operation) 
	...
	var userStore = Ext.create('Ext.data.Store', 
	{
		...
		proxy : 
		{
			type : 'ajax', api : {	read : '<c:url value="/vendor/list"/>' },
			reader : { type : 'json', root : 'data', successProperty : 'success' },
			listeners : 
			{
				exception : function(proxy, response, operation) 
	...
	{
		bodyStyle:'padding:5px', url : '<c:url value="/purchaseorder/update" />',
		items : 
		[
			{ 	title: 'Purchase Order', layout:'column', bodyStyle:'padding:10px', border:false, labelWidth : 80, monitorValid : true,
				...
				[ 
					{ 	columnWidth:.5, border:false, layout: 'anchor', defaultType: 'textfield',
						items: 
						[
							{ fieldLabel : 'Order Number', name : 'orderId', value : '<c:out value="${poBean.orderId}" />', anchor:'95%', allowBlank : false}, 
							{ fieldLabel : 'Vendor', name : 'userName', value : '<c:out value="${poBean.userName}" />', anchor:'95%', allowBlank : false,
							...
							{ fieldLabel : 'Reference Number', name : 'referenceNumber', value : '<c:out value="${poBean.referenceNumber}" />', anchor:'95%', allowBlank : false},
						]
					},
					{	columnWidth:.5, border:false, layout: 'anchor', defaultType: 'textfield',
						items: 
						[
							{ fieldLabel : 'Order Type', name : 'orderType', value : '<c:out value="${poBean.orderType}" />', anchor:'95%', allowBlank : false}, 
							{ fieldLabel : 'Order Date', name : 'orderDate', value : '<c:out value="${poBean.orderDate}" />', xtype: 'datefield', anchor:'95%', allowBlank : false},
							{ fieldLabel : 'Due Date', name : 'dueDate', value : '<c:out value="${poBean.dueDate}" />', xtype: 'datefield', anchor:'95%', allowBlank : false}
						]
					}
				]
			},
			{	border:false, bodyStyle:'padding:10px', layout: 'anchor', defaultType: 'textfield',
				items: [ { fieldLabel : 'Comments', name : 'comments', value : '<c:out value="${poBean.comments}" />', xtype: 'textareafield', anchor:'98%', allowBlank : false} ]
			},
			...
			{   border: false, defaultType: 'textfield', xtype: 'panel', flex: 1, frame: true,
				html: '<h3>Total# <c:out value="${poBean.total}" /> INR'
			}
		...
		purchaseOrderForm.getForm().submit(
		{
			...
			success : function(form, action) 
			{
				Ext.Msg.alert('Success', action.result.message, function(btn){
				    if (btn == 'ok'){
				    	window.location = "<c:url value="/purchaseorder/view/${poBean.orderId}"/>" ;
				    }
				});
			},
