...
Ext.onReady(function() 
{
	...
	{
		model : 'SalesItems', autoLoad : false,
		proxy : 
		{
			type : 'ajax',
				...
				[ 
					{
						...
						items: 
						[
							{ fieldLabel : 'Order Number', id : 'orderId', name : 'orderId', value : '<c:out value="${soBean.orderId}" />', anchor:'95%', allowBlank : false}, 
							{ fieldLabel : 'Customer', name : 'userName', value : '<c:out value="${soBean.userName}" />', anchor:'95%', allowBlank : false,
					...
					},
					{
						...
						[
							...
							{ fieldLabel : 'Order Date', name : 'orderDate', value : '<c:out value="${soBean.orderDate}" />', xtype: 'datefield', anchor:'95%', allowBlank : false},
		...
		SalesOrderForm.getForm().submit(
		{
			...
			success : function(form, action) 
			{
				Ext.Msg.alert('Success', action.result.message, function(btn){
				    if (btn == 'ok'){
				    	window.location = "<c:url value="/saleorder/view/"/>" + Ext.getCmp("orderId").getValue() ;
				    }
				});
			},
