...
Ext.onReady(function() 
{
	...
	{
		model : 'Purchase', autoLoad : false, remoteFilter : 'true', remoteSort : 'true',
		proxy : 
		{
			type : 'ajax',
	...
	{
		frame : true, height : 500, store : purchaseStore, selModel : sm, iconCls : 'icon-user',
		columns : 
		[
			{ text : 'Order Id', flex : 1, sortable : true, filterable: true, dataIndex : 'orderId', hideable: false, 
				renderer : function(value, metaData, record, rowIndex, colIndex, store) 
				{
					return '<a href="<c:url value="/purchases/view/"/>' + value + '">' + value + '</a>';
				}
			}, 
		...
		[
			{	items : 
				[
					'-',
					{ text : 'Add', itemId : 'add', handler: function() { window.location = "<c:url value="/purchases/add"/>" ;}}, 
					'-',
	...
	{   bodyStyle:'padding:5px', fieldDefaults: { labelAlign: 'left', msgTarget: 'side' }, plain: true, defaults: { bodyStyle:'padding:10px'}, activeTab: 0,
		items:
		[
			{  	title: '<c:out value="${userBean.firstName}" />', defaultType: 'textfield', items: [userDetailsForm] },
			{  	title: 'Invoices', defaultType: 'textfield', items: [purchaseList], 
				listeners: 
				{
