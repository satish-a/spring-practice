...
Ext.onReady(function() 
{
	...
	{
		store : userStore, frame : true, height : 500, selModel : sm, iconCls : 'icon-user',
		columns : 
		[
			{ text : 'Account Name', flex : 1, sortable : true, filterable: true, dataIndex : 'username',
				renderer : function(value, metaData, record, rowIndex, colIndex, store) 
				{
					return '<a href="<c:url value="/vendor/view/"/>' + value + '">' + value + '</a>';
				}
			}, 
		...
		[
			{
				...
				[
					'-',
					{ text : 'Add', itemId : 'add', 
						handler: function() { window.location = "<c:url value="/vendor/add"/>" ;}}, 
					'-',
				...
				displayMsg : 'Displaying vendors {0} - {1} of {2}', emptyMsg : "No vendors to display"
	...
	function processMarkedDeletions()
	{
		...
		if (sels.length > 0) 
		{
			...
			Ext.Msg.show(
			{
				title: 'Delete vendors?',  buttons: Ext.MessageBox.YESNO,
				msg: 'Do you want delete the selected vendors?',
				fn: function(btn)
				{
					if (btn == 'yes')
					{
	...
	function sendMarkedDeletions(users)
	{
		Ext.Ajax.request(
		{ 
			url: '<c:url value="/vendor/delete"/>',
			method:'POST',
			params: { 'users': Ext.JSON.encode(users) },
			success: function(response)
			{
				Ext.Msg.alert('Success', 'Removed Vendors Successfully');
				userStore.load();
			}
		});
	}
	...
	{
		title : 'Vendors', labelWidth : 80, frame : true, collapsible:true, collapsed:true, monitorValid : true, bodyStyle:'padding:10px',
		...
		[
			...
			{
				text : 'Show All',
				handler: function() { userStore.clearFilter();}
			} 
		]
	});
